...
def path2framenum(path):
    path = path.replace('frame-', '').replace('-0000', '')
...
def search_video_dir(video_dir):
    frames = glob.glob(os.path.join(video_dir, "*.png"), recursive=False)
...
def export_project(project, proj_filename):
    proj_path = os.path.join(project["proj_dir"], 'output', 'ebs', proj_filename + ".ebs")
...
def run(project_dir):
    ...
    frame_path = project_dir + '\output\key_frames'
    img2img_upscale_key_path = project_dir + '\output\sd_keys_upscaled'
    img2img_key_path = project_dir + '\output\sd_keys'
    ...
    if not os.path.isdir(img2img_upscale_key_path):
        print('img2img_upscale_key_path : no such dir %s' % img2img_upscale_key_path)
        ...
        print('The size of frame and img2img_key matched. use %s instead' % img2img_key_path)
